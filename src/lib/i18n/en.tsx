...
export const en: Messages = {
  ...
  common: {
    ...
    offHeadline: (what: string) => `There is not enough sample yet to estimate ${what}.`,
    offProgress: (n: string, minN: string, percent: string) =>
      `${n} of ${minN} complete days · ${percent}%`,
    offForecastDate: (day: string, missing: string) =>
      `At one day per day it switches on ${day} — ${missing} ${missing === '1' ? 'day' : 'days'} from now — assuming every day from here carries a complete record.`,
    offForecastRange: (minN: string) =>
      `It never switches on at this range: it asks for ${minN} days and the window is shorter. Switch the range to “All”.`,
    offForecastWindow:
      'The window is already full, so each new day pushes an old one out and waiting does not make it bigger. Widen the range.',
    offBindingDecisive: (field: string, without: string) =>
      `One variable is the bottleneck: “${field}”. Without it there would be ${without} complete days, which would already be enough.`,
    offBinding: (field: string, without: string, current: string) =>
      `The variable costing the most days is “${field}”: without it there would be ${without} instead of ${current}. Not enough on its own.`,
    offMeanwhile: 'What can be said with what is already here:',
